...
def load_img_name_list(dataset_path):  # 传入图片路径
    img_name_list = np.loadtxt(dataset_path, dtype=np.str) # 路径
...
class ImageDataset(data.Dataset):
    ...
    def __init__(self, root_dir, split='train', img_size=256, is_train=True,to_tensor=True):
        ...
        # self.list_path = os.path.join(self.root_dir, LIST_FOLDER_NAME, self.split+'.txt') # path+list+train+'.txt'

        # 存储所有图片标签的列表
        # self.img_name_list = load_img_name_list(self.list_path)
        if split == 'train':
            self.img_name_list = [i for i in os.listdir('/mnt/g/DataSet/LEVIR-CD/subset/train/A' ) if not  # os.listdir文件路径顺序是混乱的
            i.startswith('.')]
    ...
    def __getitem__(self, index):
        ...
        A_path = get_img_path(self.root_dir+self.split+'A',self.img_name_list[index % self.A_size])
        B_path = get_img_post_path(self.root_dir+self.split+'B',self.img_name_list[index % self.A_size])
...
class CDDataset(ImageDataset):   # 继承了上述ImageDataset 自带__len__方法
