...
将加权后的值向量经过一个线性变换 self.proj 和一个 dropout 层 self.proj_drop，得到最终的输出张量。
    ...
    def forward(self, x, H, W):
        ...
        # 由x创建Query
        q = self.q(x).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # FC (B,num_head,N,C//num_head)
        ...
        if self.sr_ratio > 1:  # 默认执行
            ...
            # 升维2倍，得到K,V
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) # FC升维 (B,N,2C)-(B,N,2,numhead,c//numhead)-(2,B,numhead,N,c//numhead)
        else:
        ...
        # 得到注意力矩阵
        attn = (q @ k.transpose(-2, -1)) * self.scale  # k转置:(B,numhead,c//numhead,N) q:(B,num_head,N,C//num_head)  kq矩阵乘法 缩放 (B,num_head,N,N)
        attn = attn.softmax(dim=-1)   # 每行softmax计算概率值作为该值与其他值的相关性    (B,num_head,N,N)
        attn = self.attn_drop(attn)   # 默认不正则化

        # 值加权
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  #(B, numhead, N, c // numhead)  - (B, N ,numhead, c // numhead)-(B,N,C)
        x = self.proj(x)                                 # FC
...
class Attention_dec(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.task_query = nn.Parameter(torch.randn(1,48,dim))  # 可学习的查询向量 [1,48,dim]
        self.sr_ratio = sr_ratio
        if sr_ratio > 1:
    ...
    def forward(self, x, H, W):
        ...
        task_q = self.task_query  # 可学习的查询向量 (1,48,dim)
        ...
        #
        q = self.q(task_q).reshape(B, task_q.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3) # (1,48,dim)-FC-(B,numhead,48,c/numhead)
